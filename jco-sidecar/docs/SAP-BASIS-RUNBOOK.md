...
- `S_RFC`, activity `16`, limited to `BAPI_USER_GETLIST` and
  `BAPI_USER_GET_DETAIL` when function-level authorization is available;
  otherwise limit the function group to `SU_USER`.
...
Verify both functions in the target system before deployment. In standard SAP
systems they belong to package `SUSR`, function group `SU_USER`:
